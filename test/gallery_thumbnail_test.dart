...
void main() {
  ...
  group('Gallery thumbnail invalidation strategy', () {
    test('should invalidate when profile updatedAt is newer than project thumbnailUpdatedAt', () {
    ...
    test('should not invalidate when profile updatedAt is older than project thumbnailUpdatedAt', () {
  ...
  group('Profile editing roundtrip', () {
    test('WatermarkProfile copyWith preserves all fields correctly', () {
      ...
      expect(updatedProfile.canvasSize?.width, originalProfile.canvasSize?.width);
    });

    test('WatermarkProfile with updated elements triggers proper timestamp', () {
      final element1 = WatermarkElement(
        id: 'e1',
        type: WatermarkElementType.text,
        ...
        textStyle: const WatermarkTextStyle(fontSize: 12),
      );
      
      final element2 = WatermarkElement(
        id: 'e2',
    ...
    test('WatermarkProject updates preserve integrity during profile binding', () {
      ...
      expect(updatedProject.thumbnailUpdatedAt!.isAfter(originalProject.thumbnailUpdatedAt!), isTrue);
    });
  });
}
